...
public class TripRestController {
    ...
    public ResponseEntity<TripVO> findByAuto(@PathVariable Long id) {
        Automobile automobile = autosService.findById(id);
        Trip trip = (Trip) tripsService.findByAuto(automobile);
        if (trip == null) {
            return new ResponseEntity<TripVO>(HttpStatus.NOT_FOUND);
        }
        TripVO result = new DozerBeanMapper().map(trip, TripVO.class);
